...
try:
    ...
    while counter < 20:
        ...
        for candidate in response.candidates: # Add response candidates to message history
        ...
        if response.function_calls and len(response.function_calls) > 0: # Check for called functions, print them, and add them to the message history
            for call in response.function_calls:
                ...
                if verbose:
                    print(f"-> {resp}")
            continue
        elif response.text and len(response.text) > 0: # If no functions were called and a final text response exists, print it and break out
                    print(response.text)
                    break
        else:
             continue
